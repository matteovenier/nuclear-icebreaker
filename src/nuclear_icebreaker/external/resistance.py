...
def calculate_resistance(
    panels, starting_speed: float, given_power: float, ice_thickness: float
) -> float:
    ...
    f_res = 0.0
    p_max = (
        ICE_FLEXURAL_STRENGTH * 1 / 1.469 * np.power(WATER_DENSITY * g * ice_thickness**5, 1 / 4)
    )
    ...
    while True:  # noqa: F405
        ...
        for panel in panels:
            ...
            R_fluid += (
                0.5
                * (WATER_DENSITY if panel.underwater else AIR_DENSITY)
                * current_speed**2
                * panel.area
                * panel.Ct
            )
        ...
        Hi = ice_thickness
        E = 5e9  # Modulus of elasticity for ice in Pascals
        ...
        # ---- R_ice ----
        R_ice = (R_C + R_B) * (1.0 + 1.4 * V / np.sqrt(g * Hi)) + R_S * (
            1.0 + 9.4 * V / np.sqrt(g * L)
        )

        f_res = R_fluid + R_ice
        if f_res < (given_power) / current_speed:
            current_speed += 0.001  # Increment speed for next iteration
        else:
            break
